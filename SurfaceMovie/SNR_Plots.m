...
h1 = Plotting_SNR(ax(1), koff_vals, rho_vals, surface_type{1}, bound_type{1}, cols);
h2 = Plotting_SNR(ax(2), koff_vals, rho_vals, surface_type{1}, bound_type{2}, hols);
Plotting_SNR(ax(3), koff_vals, rho_vals, surface_type{2}, bound_type{1}, cols)
Plotting_SNR(ax(4), koff_vals, rho_vals, surface_type{2}, bound_type{2}, hols)
Plotting_SNR_Ratio(ax(5), koff_vals, rho_vals, surface_type{1}, cols)
Plotting_SNR_Ratio(ax(6), koff_vals, rho_vals, surface_type{2}, hols)

for i=1:2
    ylabel(ax(i), 'Bound SNR')
    ylabel(ax(i+2), 'Phos SNR')
...
function [y] = Plotting_SNR(ax, koff_vals, rho_vals, surface_type, bound_type, cols)
    ...
    for koff = koff_vals
        ...
        y(i) = plot(ax, rho_vals, x,'--x','Color',cols(i,:));
...
function [line] = Plotting_SNR_Ratio(ax, koff_vals, rho_vals, surface_type, cols)
    ...
    for koff = koff_vals
        ...
        line(i) = plot(ax, rho_vals, x./y,'--x','Color',cols(i,:));
...
function [snr, mean_snr, var_snr] = Bootstrap_SNR(koff, rho, surface_type, bound_type)
    ...
    m = size(tcr_dist,2);
    k = 90;
    start = floor(0.75*m);  % Select the last 25% of time series
    ...
    while i <= 100
        ...
        snr(i) = mean(x,'all') ./ var(x,0,'all','omitnan');
