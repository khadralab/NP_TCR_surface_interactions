function mi_cell = Rho_MI_matrix(ifsave, norm)
    ...
    for t = 1:length(surfaces)
        ...
        mi = zeros(length(valence_vals), length(koff_vals));
        variance_mi = zeros(length(valence_vals), length(koff_vals));
        ...
        for i = 1:length(valence_vals)
            ...
            for j = 1:length(koff_vals)
                ...
                [mean_mi, var_mi] = bootstrap_MI(tcr_dist, rho_dist, 30, norm);
                mi(i,j) = mean_mi;
    ...
    if ifsave && norm
        save('Mutual Information/rho_nmi','mi_cell');
    elseif ifsave && ~norm
        save('Mutual Information/rho_mi','mi_cell');
    end
end