function [y] = SpecificityPlots(axes, kd_vals, rho_vals, tpc, valence, bound_type, colors)
    ...
    specMat = diff(dr_vals');
    specMat = abs(specMat);
    sensMat = diff(dr_vals);

    plot(axes(1), x_vals, specMat')
    xlabel(axes(1), 'NP Concentration')
    ylabel(axes(1), 'Specificity')
    
    axes(1).ColorOrder = colors;

    plot(axes(2), x_vals(1:end-1), sensMat)
    xlabel(axes(2), 'NP Concentration')
    ylabel(axes(2), 'Sensitivity')
    
    axes(2).ColorOrder = colors;
end
