function y = plot_DP_KPR(ax1, threshold_array, valence_vals, tpc, kp)
    ...
    linestyle = {'-','--'};
    markerstyle = {'s','d'};
    colorstyle = [hol(2,:); hol(7,:); sol(7,:); sol(2,:)];
    ...
    for j = 1:length(kp)
        ...
        for v = 1:length(valence_vals)
            ...
            for i = 1:length(koff_vals)
                koff = koff_vals(i);
                [mean_data,var_data,output_fit] = load_DR(koff, kp(j,:), rho_vals, tpc, np_radius, valence, 1);
                ...
                for k = 1:length(threshold_array)
                    ...
                    if i == length(koff_vals)
                        ...
                        pfit(k,:) = polyfit(log10(koff_vals(~isnan(P(:,k)))), log10(P(~isnan(P(:,k)),k)),1);
                        y(k,:) = polyval(pfit(k,:), xFit);

                        if ismember(tpc,[1,3,20]) && valence == 5
                            loglog(ax1, 10*koff_vals, P(:,k),'.','Color', colorstyle(j,:),'MarkerSize',10,'HandleVisibility','off')
                            hold(ax1,'on');
                            %loglog(ax1, 10*10.^xFit, 10.^ylo(k,:),'LineStyle',':','Color',colorstyle(j,:));
                            %loglog(ax1, 10*10.^xFit, 10.^yhi(k,:),"LineStyle","--",'Color',colorstyle(j,:));
                            loglog(ax1, 10*10.^xFit, 10.^y(k,:),"LineStyle","--",'Color',colorstyle(j,:));
                            grid(ax1, 'on');
                        end         
