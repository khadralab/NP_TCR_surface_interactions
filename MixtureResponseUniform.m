function [uniform_states] = MixtureResponseUniform(rho_index, mixRatio)
    ...
    cfile = ['MixtureResults/Cluster_rho',num2str(rho_total),'-',num2str(mixRatio*100),'.mat'];
    ufile = ['MixtureResults/Uniform_rho',num2str(rho_total),'-',num2str(mixRatio*100),'.mat'];
    ...
    for nsims = [1:total_sims]
        [bound_tcr, bound_np1, bound_np2, phos_tcr, uniform_states, time] = TauMixtures(tcr_params, np1_params, np2_params, sim_params);
        [time,j,~] = unique(time);
        ...
        homo_bound_np1{nsims} = [bound_np1; time];
        homo_bound_np2{nsims} = [bound_np2; time];
        homo_phos_tcr{nsims} = [phos_tcr; time];
    end
    save(ufile,'homo_bound_tcr','homo_bound_np1','homo_bound_np2','homo_phos_tcr');
